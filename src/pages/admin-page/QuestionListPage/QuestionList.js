    ...
    },
    {
      ...
      render : (record) => {
        if(record.type === 'fillblank') {
          return (
            <div>{record.content[0]}______{record.content[1]}</div>
          )
        } else {
          return (
            <div>{record.content[0]}</div>
          )
        }
      }
    },
  ...
  return (
    <BasicLayout>
      <Table
        ...
        rowKey={'_id'}
        pagination={{ showSizeChanger: false, showQuickJumper: true, total: totalPage, current: page, onChange: (page) => setPage(page) }}></Table>
